from keras.layers import Embedding,LSTM,Dropout,Dense,Layer, Softmax
...
class AttentionLayer(Layer):
  ...
  def call(self, inputs, mask=None):
    ...
    luong_score = Softmax(axis=1)(K.batch_dot(encoder_outputs, K.permute_dimensions(decoder_outputs, (0,2,1))))
    
    encoder_vector = K.sum((K.expand_dims(luong_score,axis =3))*(K.expand_dims(encoder_outputs, axis = 2)), axis = 1)
...
class NmtModel(object):
  ...
  def build(self):
    ...
    source_embeddings = Embedding(input_dim = self.vocab_source_size, output_dim = self.embedding_size, mask_zero=True)
    target_embeddings = Embedding(input_dim = self.vocab_target_size, output_dim = self.embedding_size, mask_zero=True)
    ...
    source_word_embeddings = Dropout(rate = self.embedding_dropout_rate)(source_embeddings(source_words))
    target_words_embeddings = Dropout(rate = self.embedding_dropout_rate)(target_embeddings(target_words))
    ...
    Start
    """
    # Task 2 (a.) Getting the decoded outputs
    print('\n Putting together the decoder states')
    # Getting the inititial states for the decoder, decoder_states
    # Decoder states are the hidden and cell states from the training stage
    decoder_states = [decoder_state_input_h, decoder_state_input_c]
    # Using decoder states as input to the decoder lstm to get the decoder outputs, h, and c for test time inference
    ...
    # Task 2 (b.) Adding attention if attention = True
    ...
    # Task 2 (c.) Passing the decoder_outputs_test (with or without attention) to the decoder dense layer
